...
public class src_Personaje1 : MonoBehaviour
{
    Animator anim;
    Rigidbody2D rb;
    BoxCollider2D box;
    public float fuerzaSalto = 1400f;
    public bool enPiso;
    ...
    void Update()
    {
        ...
        if (movX < 0) {
            transform.localScale = new Vector3(-0.27f, 0.27f, 0f);
        }        
        if (movX > 0) {
            transform.localScale = new Vector3(0.27f, 0.27f, 0f);
        }   
        ...
        {
            energiaActual = 0;
            //Se debe apagar el seguimiento de la camara al personaje
            GameOver();            
        }
        //Sonido de la caminata 
        if (Mathf.Abs(movX) < 0.1f && enPiso)
        {
    ...
    public void RecibirGolpe(Vector2 posicion)
    {
        ...
        else
        {
            ...
            transform.position = new Vector3(transform.position.x + 1f * direccionGolpe, transform.position.y, 0f);
